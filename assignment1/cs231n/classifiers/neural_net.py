...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        relu = lambda x: np.maximum(0, x) # Defining the reLU function 
        h = relu(X.dot(W1)) + b1  #computing h
        scores = h.dot(W2) +b2  #Computing final output as scores
        ...
        lim_scores = scores - np.max(scores)  # Limit score so max score is 0
        correct_scores = lim_scores[np.arange(N) ,y] #array of the correct scores
    ...
    # db1    
        ...
        grads['b2'] = np.sum(dscores, axis=0)  #converting (N,C) to (N,)
